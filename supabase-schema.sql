...
create table if not exists public.grow_session_condition_corrections (
  ...
  constraint grow_session_condition_corrections_note_check
    check (
...
revoke all on function public.normalize_session_condition_correction_note(text) from public;
revoke all on function public.normalize_session_condition_correction_note(text) from anon;
revoke all on function public.normalize_session_condition_correction_note(text) from authenticated;
revoke all on function public.normalize_session_condition_correction_note(text) from service_role;

revoke all on function public.project_session_condition_dimension_v2(uuid, text, timestamptz) from public;
revoke all on function public.project_session_condition_dimension_v2(uuid, text, timestamptz) from anon;
revoke all on function public.project_session_condition_dimension_v2(uuid, text, timestamptz) from authenticated;
revoke all on function public.project_session_condition_dimension_v2(uuid, text, timestamptz) from service_role;

revoke all on function public.project_canonical_session_conditions_v3(uuid, timestamptz) from public;
revoke all on function public.project_canonical_session_conditions_v3(uuid, timestamptz) from anon;
revoke all on function public.project_canonical_session_conditions_v3(uuid, timestamptz) from authenticated;
revoke all on function public.project_canonical_session_conditions_v3(uuid, timestamptz) from service_role;

revoke all on function public.get_current_session_conditions_v1(uuid, timestamptz) from public;
revoke all on function public.get_current_session_conditions_v1(uuid, timestamptz) from anon;
revoke all on function public.get_current_session_conditions_v1(uuid, timestamptz) from service_role;
grant execute on function public.get_current_session_conditions_v1(uuid, timestamptz) to authenticated;

revoke all on function public.change_current_session_conditions(uuid, uuid, jsonb, bigint) from public;
revoke all on function public.change_current_session_conditions(uuid, uuid, jsonb, bigint) from anon;
revoke all on function public.change_current_session_conditions(uuid, uuid, jsonb, bigint) from service_role;
grant execute on function public.change_current_session_conditions(uuid, uuid, jsonb, bigint) to authenticated;

revoke all on function public.correct_current_session_condition(uuid, uuid, uuid, jsonb, bigint) from public;
revoke all on function public.correct_current_session_condition(uuid, uuid, uuid, jsonb, bigint) from anon;
revoke all on function public.correct_current_session_condition(uuid, uuid, uuid, jsonb, bigint) from service_role;
grant execute on function public.correct_current_session_condition(uuid, uuid, uuid, jsonb, bigint) to authenticated;

revoke all on function public.set_current_conditions_for_unresolved_legacy(uuid, uuid, jsonb, bigint) from public;
revoke all on function public.set_current_conditions_for_unresolved_legacy(uuid, uuid, jsonb, bigint) from anon;
revoke all on function public.set_current_conditions_for_unresolved_legacy(uuid, uuid, jsonb, bigint) from service_role;
...
begin
  ...
  loop
    ...
    if seed_type not in ('unknown', 'not_applicable', 'photoperiod', 'autoflower', 'fast_flower')
      or seed_sex not in ('unknown', 'not_applicable', 'feminized', 'regular') then
