...
-- This is intentionally additive and covers the full payload in one migration
-- so PostgREST does not fail one missing column at a time.
...
create table if not exists public.seed_vault_entries (
  ...
  dev_mode_only boolean not null default false,
  mock_source text,
...
alter table public.seed_vault_entries
  ...
  add column if not exists dev_mode_only boolean not null default false,
...
set
  ...
  archived_at = case
    when coalesce(is_archived, false) then coalesce(archived_at, timezone('utc', now()))
    else null
  end,
  is_deleted = coalesce(is_deleted, false),
  deleted_at = case
    when coalesce(is_deleted, false) then coalesce(deleted_at, timezone('utc', now()))
    else null
  end,
...
begin
  if not exists (
    ...
    alter table public.seed_vault_entries
      ...
      alter column dev_mode_only set not null,
