...
alter table public.public_member_profiles
  ...
  add column if not exists growing_environments text[],
  add column if not exists favorite_germination_methods text[],
  add column if not exists grow_interests text[],
  add column if not exists grow_goals text[],
  add column if not exists favorite_breeders text[],
  add column if not exists favorite_sources text[],
  add column if not exists city text,
  add column if not exists state_province text,
  add column if not exists country text,
  add column if not exists timezone text,
  add column if not exists grow_network_discoverable boolean,
  add column if not exists connection_request_permission text,
  add column if not exists invitation_preferences jsonb,
  add column if not exists personalization_consent boolean,
  add column if not exists identity_provenance jsonb;

alter table public.public_member_profiles
  drop constraint if exists public_member_profiles_visibility_check;

-- Historical blanket identity backfill intentionally omitted. Existing values remain unchanged; new fields stay null until separately evidenced.
...
-- Historical visibility backfill intentionally omitted. Missing visibility evidence remains absent and therefore fails closed.
...
begin
  ...
  select new.user_id, field_key, 'connections'
...
/* removed dangling historical seed body */
...
drop function if exists public.get_grow_identity_v1(uuid, text);
...
drop function if exists public.search_grow_identities_v1(text, integer);
...
drop function if exists public.can_request_grow_connection_v1(uuid);
...
drop function if exists public.update_my_grow_identity_v1(jsonb);
...
drop function if exists public.get_public_identity_and_recognition(uuid);
...
-- Forward replacement of the divergent Profile Hero projection.
