-- Reassert and harden the founder/admin grow-session cleanup RPC.
--
-- This follow-up migration is intentionally self-contained because production
-- may be missing the function entirely or may have an older overload in the
-- schema cache even after earlier deploys. It does not touch CSTP, auth users,
-- admin roles, app settings, or production config tables.
