-- Safe Community Grow publication-layer reset helpers.
--
-- These functions are intentionally scoped to Community publication data:
--   - public.grow_gallery_snapshots
--   - public.grow_gallery_snapshot_likes
--   - public.community_activity rows generated by those snapshots/sessions
--
-- They do not delete or update Grow Sessions, users, profiles, Seed Vault data,
-- private session images, private notes, private reports, or session results.
...
revoke all on function public.community_grow_reset_is_authorized() from public;
...
revoke all on function public.community_grow_reset_table_count(text) from public;
...
begin
  if not public.community_grow_reset_is_authorized() then
    raise exception 'Admin or service-role access is required to preview Community Grow reset.'
      using errcode = '42501';
  end if;
...
revoke all on function public.admin_preview_community_grow_publication_reset() from public;
grant execute on function public.admin_preview_community_grow_publication_reset() to authenticated;
...
begin
  if not public.community_grow_reset_is_authorized() then
    raise exception 'Admin or service-role access is required to execute Community Grow reset.'
      using errcode = '42501';
  end if;
...
revoke all on function public.admin_execute_community_grow_publication_reset() from public;
grant execute on function public.admin_execute_community_grow_publication_reset() to authenticated;
