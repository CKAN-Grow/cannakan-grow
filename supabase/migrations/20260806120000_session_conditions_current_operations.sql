-- ICE-SC-003: forward-only Current Conditions operations and unresolved-legacy declaration.
-- This migration preserves all existing canonical evidence and replaces only the
-- future Product mutation surface bounded by IC-SC-001C.

alter table public.grow_session_conditions_authority
  drop constraint if exists grow_session_conditions_authority_source_check;
alter table public.grow_session_conditions_authority
  add constraint grow_session_conditions_authority_source_check
...
alter table public.grow_session_condition_periods
  drop constraint if exists grow_session_condition_periods_source_check;
alter table public.grow_session_condition_periods
  add constraint grow_session_condition_periods_source_check
...
alter table public.grow_session_condition_corrections
  add column if not exists correction_note text;

alter table public.grow_session_condition_corrections
  drop constraint if exists grow_session_condition_corrections_note_check;
alter table public.grow_session_condition_corrections
  add constraint grow_session_condition_corrections_note_check
  check (
    correction_note is null
    or (
      char_length(correction_note) <= 2000
      and translate(correction_note, E'\n\t', '') !~ '[[:cntrl:]]'
      and position(chr(127) in correction_note) = 0
    )
  );

create or replace function public.normalize_session_condition_correction_note(
  p_note text
)
returns text
language plpgsql
...
begin
  ...
  normalized := btrim(replace(replace(p_note, E'\r\n', E'\n'), E'\r', E'\n'));
  if normalized = '' then
    return null;
  end if;
  if char_length(normalized) > 2000
    or translate(normalized, E'\n\t', '') ~ '[[:cntrl:]]'
    or position(chr(127) in normalized) > 0 then
    raise exception 'The correction note contains an unauthorized character or exceeds 2,000 characters.'
      using errcode = '22023';
  end if;
  return normalized;
end;
...
declare
  ...
  forward_unavailable boolean := false;
begin
  select * into commencement_row
  ...
  loop
    ...
    if authority_row.authority_source = 'forward_legacy_declaration'
      and defined_at < authority_row.cutover_at then
    ...
    elsif commencement_row.session_id is null and authority_row.session_id is null then
      condition_result := jsonb_build_object(
        'dimension', dimension_name,
        'status', 'unresolved',
        'value', null,
    ...
    elsif commencement_row.session_id is not null
      and authority_row.session_id is null
      and defined_at < commencement_row.commenced_at then
      condition_result := jsonb_build_object(
        'dimension', dimension_name,
      ...
      where condition_period.session_id = p_session_id
        ...
        and (condition_period.effective_end is null or condition_period.effective_end > defined_at);
      ...
      if applicable_count = 1 then
        ...
        condition_result := jsonb_build_object(
          'dimension', dimension_name,
          'status', case
            when authority_row.authority_source = 'forward_legacy_declaration' then 'unavailable'
            else 'absent'
          end,
          'value', null,
          'other_text', '',
          ...
          'effective_start', null,
          'effective_end', null,
          'source_kind', authority_row.authority_source
        );
      else
        condition_result := jsonb_build_object(
          'dimension', dimension_name,
  ...
  return jsonb_build_object(
    ...
    'growing_commencement_status', case when commencement_row.session_id is null then 'unresolved' else 'authoritative' end,
    'growing_commenced_at', commencement_row.commenced_at,
    'defined_at', defined_at,
    'earlier_conditions_status', case when forward_unavailable then 'unavailable' else null end,
...
revoke all on function public.change_session_condition(uuid, uuid, text, text, text, timestamptz, bigint) from authenticated;
revoke all on function public.correct_session_condition(uuid, uuid, uuid, jsonb, bigint) from authenticated;
revoke all on function public.migrate_session_conditions(uuid, uuid, timestamptz) from authenticated;
...
revoke all on function public.correct_current_session_condition(uuid, uuid, uuid, jsonb, bigint) from public, anon, service_role;
grant execute on function public.correct_current_session_condition(uuid, uuid, uuid, jsonb, bigint) to authenticated;
revoke all on function public.set_current_conditions_for_unresolved_legacy(uuid, uuid, jsonb, bigint) from public, anon, service_role;
grant execute on function public.set_current_conditions_for_unresolved_legacy(uuid, uuid, jsonb, bigint) to authenticated;

comment on function public.change_current_session_conditions(uuid, uuid, jsonb, bigint) is
  'Atomic owner-scoped one-or-two-dimension Current Conditions change with canonical boundary, changed-set, revision, and retry identity.';
comment on function public.correct_current_session_condition(uuid, uuid, uuid, jsonb, bigint) is
  'Owner-scoped prospective value-only correction with immutable applicability boundaries and optional bounded provenance note.';
comment on function public.set_current_conditions_for_unresolved_legacy(uuid, uuid, jsonb, bigint) is
  'Atomic forward-only declaration for eligible unresolved legacy Sessions; earlier conditions remain unavailable.';

-- The explicit scalar helper keeps projection evaluation independent of row-type
-- assignment and makes the two-dimension result deterministic for every state.
...
revoke all on function public.get_current_session_conditions_v1(uuid,timestamptz) from public,anon,service_role;
grant execute on function public.get_current_session_conditions_v1(uuid,timestamptz) to authenticated;

comment on function public.get_current_session_conditions_v1(uuid,timestamptz) is
  'Owner-scoped deterministic Current Conditions projection including forward-only legacy applicability and earlier unavailability.';

revoke all on function public.normalize_session_condition_correction_note(text) from public,anon,authenticated,service_role;
revoke all on function public.project_session_condition_dimension_v2(uuid,text,timestamptz) from public,anon,authenticated,service_role;
revoke all on function public.project_canonical_session_conditions_v3(uuid,timestamptz) from public,anon,authenticated,service_role;
