...
The Grow Evidence Engine (GEE) is the canonical analytics layer for Grow. It is the only subsystem that may transform operational Grow data into analytics for product surfaces such as Seed Explorer, Source Explorer, Community summaries, rankings, reports, admin dashboards, recommendations, and future AI features.

## Architecture Overview

Operational systems own operational data. The GEE reads that data, validates it, normalizes it, classifies it, aggregates it, calculates analytics, versions the output, and exposes read-only analytics payloads.

The GEE does not mutate operational records. Session creation, completion, deletion, publication, snapshot management, Seed Vault records, and account settings remain owned by their existing domain flows.
...
The lifecycle resolver determines whether a session participates. The GEE then
...
New analytics and new data-quality measures must be added to the GEE first,
versioned there, and only then exposed to consumers.

## Three-Contract Architecture (Phase 1)

There is exactly one Grow Evidence Engine. Phase 1 exposes three stable,
...
The Global contract preserves the already released anonymous GEE payload under
...
Global `analytics` preserves the complete released GEE payload: completed
...
Identify Global, Owner, or Community scope
        ↓
Extend the correct GEE contract
        ↓
...
The GEE sits on top of the canonical Grow Session lifecycle resolver.
...
Warning, and 80% Needs Attention. Changing these thresholds changes the GEE
...
It must return the same GEE payload and should not become a second analytics source.

## Data Lineage

The GEE may read from:
...
Current and future consumers must read GEE output or lifecycle resolver output:
...
UI components render GEE output. They do not independently calculate aggregate analytics.
...
The diagnostic may expose session identifiers and names to admins for audit. Public GEE analytics must remain anonymous.
...
| Seed Explorer | ✅ Uses GEE | Reads the cached canonical GEE payload; the local completed-session aggregate fallback was removed. |
| Source Explorer | ✅ Uses GEE | Headlines and source records read the same cached canonical GEE payload. |
...
| Grow Intelligence Health | ✅ Uses GEE | Renders GEE engine, schema, data-quality version, score, status, and breakdown without reinterpretation. |
| Rankings | ✅ Uses Community Contract | Community ranking pages preserve GEE order and render GEE rank fields. Home is outside Group B and retains its documented teaser wrapper. |
| Leaderboards | ✅ Uses Community Contract | Community source, variety, and contributor leaderboards render GEE rows and ranks. |
| Community Confidence | ✅ Uses Community Contract | Public Community and report confidence labels and percentages originate in GEE. |
| Recommendations | ✅ Uses scoped GEE Contracts | Canonical recommendations and contexts include evidence, confidence, scope, versions, and generation time; UI code only selects and presents returned items. |
| AI integration hooks | ✅ Uses scoped GEE Contracts | One explicit-scope adapter whitelists Global, Community, Owner, or authorized Admin Owner context and returns unavailable/unauthorized states without raw-data fallback. |
| Cached aggregate / RPC compatibility | ✅ Uses GEE | The cache contains normalized GEE output; the legacy Explorer RPC is a calculation-free GEE-delegating wrapper. |

### Permanent development rule

**No analytics without GEE.** A new analytics requirement must extend and
version the GEE before any consumer renders it. New UI code must not introduce
...
outside GEE only when they do not derive analytics.
...
| --- | --- | --- | --- | --- | --- |
| Home | GEE previews plus owner sessions and public snapshots / mixed | Owner totals; gallery rankings | Owner: active/completed, totals, trends; Community: rankings | High; keep owner and public caches separate | A, then B |
...
| Seed Explorer | Global GEE compatibility RPC / anonymous | None after audit cleanup | Global: released seed records and totals | Low; preserve anonymous response | Existing |
| Source Explorer | Global GEE compatibility RPC / anonymous | None after audit cleanup | Global: released source records and attribution | Low; preserve headline parity | Existing |
| Variety Reports | Global GEE plus public snapshots / anonymous/public | Supporting evidence filters | Global: variety aggregate; Community: public evidence | Medium; separate truth from evidence | B |
...
| Grow Intelligence Health | GEE and contract diagnostics / admin | None | All metadata and diagnostic status | Low; read-only admin page | Existing |
...
| Community Confidence | GEE plus local public scoring / anonymous/public | Directory confidence | Global or Community confidence by evidence scope | Medium; never mix scopes | B |
...
| Cached aggregates / wrappers | Normalized GEE cache and delegating RPCs / anonymous | No calculations in canonical wrappers | Global now; separate keyed caches for Owner/Community later | Medium; scope cache keys and invalidation | Foundation |
...
on the full eligible GEE population, never a client-visible slice.
...
preview helper. The Global Explorer compatibility RPC remains a delegating GEE
...
time, and an explicit evidence state. The single `getGeeAiAnalyticsContext()`
...
`get_grow_intelligence_engine_analytics()` SQL wrapper delegates to Global GEE;
...
existing GEE contract if required → version the contract/schema → consume the
...
therefore no GEE RPC response. Configure `CANNAKAN_SUPABASE_URL` and
...
state until GEE configuration is valid.

### Performance findings

- GEE loads once during signed-in or signed-out hydration and refreshes after
  analytics-relevant session mutations. No GEE polling loop exists.
...
- Normalized GEE payloads are marked and reused so Explorer renders do not
...
- Rapid consecutive session mutations can initiate multiple refresh requests.
  A future non-semantic optimization may coalesce in-flight GEE refreshes.
...
3. Extend the shared GEE helper and the correct contract.
4. Version the contract/schema when behavior changes.
5. Update admin contract diagnostics.
6. Point consumers at the scoped GEE output.
