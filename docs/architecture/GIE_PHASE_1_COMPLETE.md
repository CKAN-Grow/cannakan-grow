# Grow Intelligence Engine (GIE) — Phase 1 Complete
...
> [!IMPORTANT]
> The Grow Intelligence Engine is the single canonical analytics platform for
> Grow. Future analytics must extend a versioned GIE contract before they are
...
Phase 1 established the Grow Intelligence Engine (GIE) as the application's
...
GIE now exposes three stable, privacy-scoped contracts:
...
rankings, confidence, or data quality. They request the correct GIE contract
and render its canonical output. Operational workflows continue to own and
mutate operational records; GIE remains a read-only analytical layer over that
data.
...
| GIE | Normalizes eligible evidence and calculates canonical analytics. |
...
There is exactly one Grow Intelligence Engine. Global, Owner, and Community are
contracts over that engine—not independent analytics implementations. Shared
logic belongs in GIE helpers so formulas cannot drift between product areas.

### Read-only Analytics Layer

GIE reads operational data and returns analytics. It does not create, edit,
...
lifecycle resolver answers, “Should this session participate?” GIE answers,
“What intelligence follows from eligible data?” UI code answers only, “How
should the canonical value be presented?”

### Canonical Lifecycle

Every session-based GIE calculation uses the canonical Grow Session Lifecycle
...
data-quality measure must originate in the correct GIE contract. UI components
...
The lifecycle resolver and GIE answer different questions:
...
failure, mock/test, analytics-exclusion, and timeline-validity decisions. GIE
...
> added or corrected. Consumers must always read the live GIE payload.
...
Data quality is canonical analytics, not a presentation-layer opinion. GIE
...
GIE also audits invalid seed counts, germinated-count bounds, orphaned
...
> [!CAUTION]
> ### Rule #1 — No analytics without GIE
>
...
3. Extend and version GIE first when fields are missing.
...
Future analytics must extend and consume GIE.
```
