...
> [!IMPORTANT]
> Grow Evidence Engine v1 is the single canonical analytics platform for
...
Grow Evidence Engine (GEE) v1 centralizes Grow's analytics, confidence,
...
analytics context now originate in GEE. UI components render normalized
...
| Grow Evidence Engine | Normalizes eligible evidence and produces canonical metrics, rankings, confidence, recommendations, and data quality. |
...
boundaries over the same Grow Evidence Engine.
...
GEE owns data-quality measurement. Consumers do not infer missing attribution,
...
- GEE and contract diagnostics remain protected by existing Admin or
...
GEE uses additive, explicit versioning:
...
- Extend the applicable existing GEE contract when a canonical field is
  missing.
...
Determine Global, Owner, or Community scope
        │
        ▼
Extend the existing GEE contract if required
        │
        ▼
...
contracts. When a feature requires new intelligence, the correct existing GEE
