...
assert(seedMetrics.includes("aggregate.totalSeedsTested"), "Seed Explorer must read total seeds directly from the GEE aggregate.");
assert(sourceMetrics.includes("aggregate.totalSeedsTested"), "Source Explorer must read total seeds directly from the GEE aggregate.");
...
[
  ...
  assert(normalizer.includes(field), `GEE payload normalizer is missing ${field}.`);
});
...
assert(dataHealth.includes("aggregate.dataQualityScore") && dataHealth.includes("aggregate.dataQualityStatus"), "Grow Intelligence Health must render the canonical GEE score and status.");
...
[
  ...
  assert(baseMigration.includes(`'${field}'`), `Base GEE payload is missing ${field}.`);
  assert(qualityMigration.includes(`'${field}'`), `Forward GEE migration is missing ${field}.`);
});

assert(qualityMigration.includes("grow_intelligence_engine_config"), "GEE source attribution thresholds must be configurable.");
assert(qualityMigration.includes("source_attribution_healthy_threshold") && qualityMigration.includes("source_attribution_warning_threshold") && qualityMigration.includes("source_attribution_needs_attention_threshold"), "GEE must expose the 95/90/80 attribution threshold configuration.");
assert(qualityMigration.includes("'Healthy'") && qualityMigration.includes("'Warning'") && qualityMigration.includes("'Needs Attention'"), "GEE must classify source attribution health canonically.");
...
assert(healthMigration.includes("'schema_version', '2026-07-13.3'"), "Scoring migration must increment the GEE schema version.");
...
assert(healthMigration.includes("source_deduction + unknown_variety_deduction"), "The canonical score must be the deterministic sum of GEE deductions.");
...
console.log("GEE canonical metrics and data-quality regression checks passed.");
