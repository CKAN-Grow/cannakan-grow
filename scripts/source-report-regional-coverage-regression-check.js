...
assert(migration.includes("order by item.published_at desc"), "Canonical activity must preserve reverse chronological ordering in GIE.");
assert(migration.includes("'96_100', '96–100%'") && migration.includes("'91_95', '91–95%'") && migration.includes("'86_90', '86–90%'") && migration.includes("'below_85', 'Below 85%'"), "GIE must define all canonical germination distribution buckets.");
assert(migration.includes("'start_percent', start_percent") && migration.includes("'end_percent', end_percent"), "GIE must provide chart-ready canonical donut boundaries.");
...
const adapterStart = app.indexOf("function normalizeGieCommunityAnalyticsPayload");
...
assert(!report.includes("GIE Confidence</span>") && !report.includes("report.confidence?.percent"), "Public Source Report UI must not expose numeric confidence.");
