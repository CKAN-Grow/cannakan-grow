...
const ownerLoader = between(app, "async function loadGieOwnerAnalytics", "function normalizeGieCommunityAnalyticsPayload");
...
const ai = between(app, "async function getGieAiAnalyticsContext", "async function refreshUserSessionsAfterSave");
...
assert(recommendations.includes("recommendationContexts") && !recommendations.includes("getSessions(") && !recommendations.includes(".sort("), "Recommendation inputs/order must originate in GIE.");
...
assert(app.includes("gieGroupCAdoptionDiagnostics") && app.includes('data-gie-group-c-adoption="true"'), "Grow Intelligence Health must render Phase 2C diagnostics.");
...
assert(docs.includes("20 of 20") && docs.includes("(100%)") && docs.includes("getGieAiAnalyticsContext()"), "Final adoption documentation is stale.");
assert(!/SUPABASE_SERVICE_ROLE_KEY|SUPABASE_SECRET_KEY/.test(app), "Browser code must not contain secret or service-role keys.");

console.log("GIE Phase 2C final adoption regression checks passed.");
